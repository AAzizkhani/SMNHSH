...
void operation::set_bus_line1(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        if (saveVehicles[temp_f].get_vis()[i] == false)
        {
            ...
            else if (dir[tem_in].distance + bbline1[index - 1] < dir[temp_f].distance)
            {
                dir[temp_f].distance = dir[tem_in].distance + bbline1[index - 1];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < bline1.size() - 1)
    {
        ...
        if (saveVehicles[temp_f].get_vis()[i] == false)
        {
            ...
            else if(dir[tem_in].distance + bbline1[index] < dir[temp_f].distance)
            {
                dir[temp_f].distance = dir[tem_in].distance + bbline1[index] ;
                    dir[temp_f].direct = dir[tem_in].direct;
                    dir[temp_f].line = dir[tem_in].line;
                    dir[temp_f].type = dir[tem_in].type;
                    dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_bus_line2(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        if (saveVehicles[temp_f].get_vis()[i] == false)
        {
            ...
            else if(dir[tem_in].distance + bbline2[index - 1]  < dir[temp_f].distance)
            {
                    dir[temp_f].distance = dir[tem_in].distance + bbline2[index - 1];
                    dir[temp_f].direct = dir[tem_in].direct;
                    dir[temp_f].line = dir[tem_in].line;
                    dir[temp_f].type = dir[tem_in].type;
                    dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < bline2.size() - 1)
    {
        ...
        if (saveVehicles[temp_f].get_vis()[i] == false)
        {
            ...
            else if(dir[tem_in].distance + bbline2[index] < dir[temp_f].distance)
            {
               
                    dir[temp_f].distance = dir[tem_in].distance + bbline2[index] ;
                    dir[temp_f].direct = dir[tem_in].direct;
                    dir[temp_f].line = dir[tem_in].line;
                    dir[temp_f].type = dir[tem_in].type;
                    dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_bus_line3(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + bbline3[index - 1]< dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + bbline3[index - 1];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < bline3.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + bbline3[index] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + bbline3[index] ;
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_sub_line1(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ssline1[index - 1] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ssline1[index - 1] ;
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline1.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ssline1[index] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ssline1[index] ;
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_sub_line2(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        if (saveVehicles[temp_f].get_vis()[i] == false)
        {
            ...
            else if(dir[tem_in].distance + ssline2[index - 1] < dir[temp_f].distance)
            {
                    dir[temp_f].distance = dir[tem_in].distance + ssline2[index - 1];
                    dir[temp_f].direct = dir[tem_in].direct;
                    dir[temp_f].line = dir[tem_in].line;
                    dir[temp_f].type = dir[tem_in].type;
                    dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline2.size() - 1)
    {
        ...
        if (saveVehicles[temp_f].get_vis()[i] == false)
        {
            ...
            else if(dir[tem_in].distance + ssline2[index]  < dir[temp_f].distance)
            {
                    dir[temp_f].distance = dir[tem_in].distance + ssline2[index];
                    dir[temp_f].direct = dir[tem_in].direct;
                    dir[temp_f].line = dir[tem_in].line;
                    dir[temp_f].type = dir[tem_in].type;
                    dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_sub_line3(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        if (saveVehicles[temp_f].get_vis()[i] == false)
        {
            ...
            else if(dir[tem_in].distance + ssline3[index - 1] < dir[temp_f].distance)
            {
                    dir[temp_f].distance = dir[tem_in].distance + ssline3[index - 1];
                    dir[temp_f].direct = dir[tem_in].direct;
                    dir[temp_f].line = dir[tem_in].line;
                    dir[temp_f].type = dir[tem_in].type;
                    dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline3.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ssline3[index] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ssline3[index];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_sub_line4(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ssline4[index - 1]< dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ssline4[index - 1] ;
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline4.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ssline4[index] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ssline4[index];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_taxi_line1(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline1[index - 1] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ttline1[index - 1];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline1.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline1[index] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ttline1[index];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_taxi_line2(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline2[index - 1]< dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ttline2[index - 1] ;
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline2.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline2[index] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ttline2[index] ;
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_taxi_line3(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline3[index - 1]< dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ttline3[index - 1];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline3.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline3[index]  < dir[temp_f].distance)
        {
            dir[temp_f].distance = dir[tem_in].distance + ttline3[index];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
...
void operation::set_taxi_line4(unordered_map<string, int> &t, int index, int tem_in, vector<int> timeoftype, saveDirect dir[V], int count, int i)
{
    ...
    if (index > 0)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline4[index - 1] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ttline4[index - 1] ;
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
    ...
    if (index < sline4.size() - 1)
    {
        ...
        else if(!saveVehicles[temp_f].get_vis()[i] && dir[tem_in].distance + ttline4[index] < dir[temp_f].distance)
        {
                dir[temp_f].distance = dir[tem_in].distance + ttline4[index];
                dir[temp_f].direct = dir[tem_in].direct;
                dir[temp_f].line = dir[tem_in].line;
                dir[temp_f].type = dir[tem_in].type;
                dir[temp_f].direct.push_back(search(temp_f, t));
